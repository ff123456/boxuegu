define(['jquery', 'header', 'loading', 'aside', 'template','nprogress', 'jqueryForm', 'datepicker', 'datepickerCN'], function ($, undefined, undefined, undefined, template,nprogress, undefined, undefined, undefined) {

  /**
   * 我们这里的js，作用于两个页面，一个是讲师添加，一个讲师编辑，
  ...
  /**
   * 提取查询字符串中的指定值：
   * 1：先去掉查询字符串的？号location.search.slice(1);
   * 2：使用&符号劈开这个字符串，得到一个数组，里面储存每一个单独的查询字符串的key和value。
   * 3：然后遍历这个数组，每一个单独字符串继续使用=号劈开，提取每一个查询字符串的key与value
   * 4：把这些查询字符串的key与value储存到一个对象中。方便使用
   */
  ...
  // 添加讲师。成功后跳转到讲师列表页
  function teacherAdd() {
    // 把模板渲染到页面中，因为是添加，所以不需要回显数据，那么传递一个空对象即可
    $('.teacher').html(template('teacher-add-edit-tpl', {}));

    // 页面渲染完毕，初始化日期插件、
    initDatepicker();

     // 利用form表单提交插件监听事件，发送ajax请求
